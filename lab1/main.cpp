...
bool connect_graph(const std::vector<node_id_t>& connected_start_nodes, const List& adjacent_list){
    for (node_id_t node: connected_start_nodes) {
        for (int i = 0; i < adjacent_list.size(); i++) {
            if (node == adjacent_list.get().second[i].n1) {
                std::string edge_text = std::to_string(adjacent_list.get().second[i].n2) + " " +
                                        std::to_string(adjacent_list.get().second[i].n1) + " " +
                                        std::to_string(adjacent_list.get().second[i].weight) +
                        adjacent_list.get().second[i].description;
                std::cout << "Adding '" << edge_text << "' to make a connected graph..." << std::endl;

                if(!edge_to_file("C:\\Users\\angel\\OneDrive\\Dokument\\DT046G Datastrukturer och Algoritmer\\lab1\\edges.txt", edge_text)){
...
int main(){
    List adjacent_list("C:\\Users\\angel\\OneDrive\\Dokument\\DT046G Datastrukturer och Algoritmer\\lab1\\edges.txt");
